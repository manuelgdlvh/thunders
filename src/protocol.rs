...
pub fn connect(
    ...
    session_manager: &SessionManager,
) -> Option<(Arc<PlayerContext>, UnboundedReceiver<Vec<u8>>)> {
    match message {
        InputMessage::Connect { id } => {
            let player_cxt = Arc::new(PlayerContext::new(id));
            Some((player_cxt, session_manager.connect(id)))
        }
        _ => None,
    }
}
...
impl SessionManager {
    ...
    pub fn subscribe(&self, player_id: u64, type_: String, id: String) {
        if let Ok(mut subscriptions) = self.subscriptions.write() {
            let subscriptions = subscriptions.get_mut(&player_id).unwrap();
            subscriptions
                .entry(type_)
    ...
    pub fn unsubscribe_all(&self, player_id: u64) -> Option<HashMap<String, Vec<String>>> {
        self.subscriptions.write().unwrap().remove(&player_id)
    }
